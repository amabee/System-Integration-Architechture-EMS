    ...
    public partial class admin_login : Form
    {
        ...
        private void bunifuButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                command.Parameters.AddWithValue("@user_role", "admin");
                MySqlDataReader mdr = command.ExecuteReader();
                if (mdr.Read())
                {
                    mdr.Close();  // close the datareader here
                                  //QUERIES
                    storedUsername = txtUsername.Text;
            ...
            try
            {
                if(connection.State != ConnectionState.Open)
                {
                    ...
                    else
                    {
                        ...
                        string selectQuery = "SELECT * FROM user_info WHERE username = @username AND password = @password;";
                        MySqlCommand command = new MySqlCommand(selectQuery, connection);
                        command.Parameters.AddWithValue("@username", storedUsername);
